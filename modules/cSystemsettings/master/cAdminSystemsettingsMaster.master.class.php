...
class cAdminSystemsettingsMaster extends cModule {
		var $template = 'admin_templates';
		var $navbar_title = 'Systemeinstellungen';
		var $navbar_id = 0;
		var $errors = array();
		...
		public static function setExecutionalHooks() {
				...
				//check the rights..
				if(false === cAccount::adminrightCheck('cAdminSystemsettings', 'USE_MODULE', (int)$_SESSION['user_id'])) {
		...
		function process() {
				...
				$cAdmin->appendBreadcrumb('Admin Systemeinstellungen', 'index.php?s=cAdminSystemsettings');
				...
				switch($this->action) {
						...
						case 'confirm_delete':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminSystemsettings&amp;action=delete&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'edit':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminSystemsettings&amp;action=update&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'new':
								$this->initData();
								$this->data['url'] = 'index.php?s=cAdminSystemsettings&amp;action=create';
								$cAdmin->appendBreadcrumb('Neu', '');
								$this->navbar_title = 'Neu';
								break;
						default:
		...
		private function delete() {
				...
				if($button_do_not_delete !== NULL && $button_do_not_delete === 'not_delete') {
						header('Location: index.php?s=cAdminSystemsettings&error=331');
						die;
				}
				
				//delete button
				if($button_delete !== NULL && $button_delete === 'delete') {
						cSystemsettings::deleteById($this->data['data']['id']);
						
						header('Location: index.php?s=cAdminSystemsettings&success=332');
						die;
				}
    ...
    ///////////////////////////////////////////////////////////////////
		...
		public function getTemplatePath() {
    		return 'modules/cSystemsettings/';
    }
		...
		public function update() {
				...
				if(0 == $id) {
						header('Location: index.php?s=admincAdminSystemsettings&error=305');
						die;
				}
        ...
        $data['id'] = $id;
                ...
                $data['title'] = core()->getPostVar('title');
     		        $data['setting_key'] = core()->getPostVar('setting_key');
     		        $data['value'] = core()->getPostVar('value');
     		
				
				if(NULL === cSystemsettings::loadById((int)$data['id'])) {
						header('Location: index.php?s=cAdminSystemsettings&error=306');
						die;
				}
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminSystemsettings&error=307');
						die;
				}
	
				header('Location: index.php?s=cAdminSystemsettings&action=edit&id=' . $id . '&success=308');
				die;
		}
		...
		function create() {
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminSystemsettings&error=309');
						die;
				}
	
				header('Location: index.php?s=cAdminSystemsettings&action=edit&id=' . $id . '&success=310');
				die;
		}
		...
		public function loadList() {
				return cSystemsettings::loadList();
		}
		...
		public function save($data) {
				...
				if(0 === $id) {
						return cSystemsettings::createInDB($data);
				}
				
				cSystemsettings::updateInDB($id, $data);
		...
		public function getContent() {
				$id = (int)core()->getGetVar('id');
				$this->data['data'] = cSystemsettings::loadById($id);
		}
    ...
    public static function setAdditionalHooks() {
		...
		public static function addMenuBarEntries() {
		        ...
		        $cAdmin->registerAdminMenuEntry($admin_menu_entry_path, 'index.php?s=cAdminSystemsettings');
				}
		}
