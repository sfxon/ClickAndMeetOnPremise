...
class cAdminMvCalendarColorsMaster extends cModule {
		var $template = 'admin_templates';
		var $navbar_title = 'Kalender-Farben';
		var $navbar_id = 0;
		var $errors = array();
		...
		public static function setExecutionalHooks() {
				...
				//check the rights..
				if(false === cAccount::adminrightCheck('cAdminMvCalendarColors', 'USE_MODULE', (int)$_SESSION['user_id'])) {
		...
		function process() {
				...
				$cAdmin->appendBreadcrumb('Admin Kalender-Farben', 'index.php?s=cAdminMvCalendarColors');
				...
				switch($this->action) {
						...
						case 'confirm_delete':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminMvCalendarColors&amp;action=delete&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'edit':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminMvCalendarColors&amp;action=update&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'new':
								$this->initData();
								$this->data['url'] = 'index.php?s=cAdminMvCalendarColors&amp;action=create';
								$cAdmin->appendBreadcrumb('Neu', '');
								$this->navbar_title = 'Neu';
								break;
						default:
		...
		private function delete() {
				...
				if($button_do_not_delete !== NULL && $button_do_not_delete === 'not_delete') {
						header('Location: index.php?s=cAdminMvCalendarColors&error=331');
						die;
				}
				
				//delete button
				if($button_delete !== NULL && $button_delete === 'delete') {
						cMvCalendarColors::deleteById($this->data['data']['id']);
						
						header('Location: index.php?s=cAdminMvCalendarColors&success=332');
						die;
				}
    ...
    ///////////////////////////////////////////////////////////////////
		...
		public function getTemplatePath() {
    		return 'modules/cMvCalendarColors/';
    }
		...
		public function update() {
				...
				if(0 == $id) {
						header('Location: index.php?s=admincAdminMvCalendarColors&error=305');
						die;
				}
        ...
        $data['id'] = $id;
				...
				if(NULL === cMvCalendarColors::loadById((int)$data['id'])) {
						header('Location: index.php?s=cAdminMvCalendarColors&error=306');
						die;
				}
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminMvCalendarColors&error=307');
						die;
				}
	
				header('Location: index.php?s=cAdminMvCalendarColors&action=edit&id=' . $id . '&success=308');
				die;
		}
		...
		function create() {
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminMvCalendarColors&error=309');
						die;
				}
	
				header('Location: index.php?s=cAdminMvCalendarColors&action=edit&id=' . $id . '&success=310');
				die;
		}
		...
		public function loadList() {
				return cMvCalendarColors::loadList();
		}
		...
		public function save($data) {
				...
				if(0 === $id) {
						return cMvCalendarColors::createInDB($data);
				}
				
				cMvCalendarColors::updateInDB($id, $data);
		...
		public function getContent() {
				$id = (int)core()->getGetVar('id');
				$this->data['data'] = cMvCalendarColors::loadById($id);
		}
    ...
    public static function setAdditionalHooks() {
		...
		public static function addMenuBarEntries() {
		        ...
		        $cAdmin->registerAdminMenuEntry($admin_menu_entry_path, 'index.php?s=cAdminMvCalendarColors');
				}
		}
