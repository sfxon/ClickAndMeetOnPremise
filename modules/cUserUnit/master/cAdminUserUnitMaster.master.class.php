...
class cAdminUserUnitMaster extends cModule {
		var $template = 'admin_templates';
		var $navbar_title = 'Abteilung/Mitarbeiter/Team';
		var $navbar_id = 0;
		var $errors = array();
		...
		public static function setExecutionalHooks() {
				...
				//check the rights..
				if(false === cAccount::adminrightCheck('cAdminUserUnit', 'USE_MODULE', (int)$_SESSION['user_id'])) {
		...
		function process() {
				...
				$cAdmin->appendBreadcrumb('Admin Abteilung/Mitarbeiter/Team', 'index.php?s=cAdminUserUnit');
				...
				switch($this->action) {
						...
						case 'confirm_delete':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminUserUnit&amp;action=delete&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'edit':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminUserUnit&amp;action=update&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'new':
								$this->initData();
								$this->data['url'] = 'index.php?s=cAdminUserUnit&amp;action=create';
								$cAdmin->appendBreadcrumb('Neu', '');
								$this->navbar_title = 'Neu';
								break;
						default:
		...
		private function delete() {
				...
				if($button_do_not_delete !== NULL && $button_do_not_delete === 'not_delete') {
						header('Location: index.php?s=cAdminUserUnit&error=331');
						die;
				}
				
				//delete button
				if($button_delete !== NULL && $button_delete === 'delete') {
						cUserUnit::deleteById($this->data['data']['id']);
						
						header('Location: index.php?s=cAdminUserUnit&success=332');
						die;
				}
		...
		public function drawEditor() {
				...
				$eventLocations = $iEventLocations->loadList();
        ...
        $renderer->setTemplate($this->template);
				...
				$renderer->assign('EVENT_LOCATIONS', $eventLocations);
    ...
    ///////////////////////////////////////////////////////////////////
		...
		function drawList() {
				$iEventLocations = new cEventLocations();
				$eventLocations = $iEventLocations->loadIndexedList();
				
				/*
				$iCmAppointmentStatus = new cCmAppointmentStatus();
				$appointmentStati = $iCmAppointmentStatus->loadIndexedList();
				*/
        ...
        $renderer->setTemplatePath($this->getTemplatePath());
				...
				$renderer->assign('EVENT_LOCATIONS', $eventLocations);
				//$renderer->assign('APPOINTMENT_STATI', $appointmentStati);
		...
		public function update() {
				...
				if(0 == $id) {
						header('Location: index.php?s=admincAdminUserUnit&error=305');
						die;
				}
        ...
        $data['id'] = $id;
				...
				if(NULL === cUserUnit::loadById((int)$data['id'])) {
						header('Location: index.php?s=cAdminUserUnit&error=306');
						die;
				}
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminUserUnit&error=307');
						die;
				}
	
				header('Location: index.php?s=cAdminUserUnit&action=edit&id=' . $id . '&success=308');
				die;
		}
		...
		function create() {
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminUserUnit&error=309');
						die;
				}
	
				header('Location: index.php?s=cAdminUserUnit&action=edit&id=' . $id . '&success=310');
				die;
		}
		...
		public function save($data) {
				...
				if(0 === $id) {
						return cUserUnit::createInDB($data);
				}
				
				cUserUnit::updateInDB($id, $data);
		...
		public function getContent() {
				$id = (int)core()->getGetVar('id');
				$this->data['data'] = cUserUnit::loadById($id);
		}
		...
		public static function addMenuBarEntries() {
				...
				if(false !== $cAdmin) {
						$admin_menu_entry_path = array(
								...
								array(
										'position' => 20,
										'title' => 'Abteilung/Mitarbeiter/Team'
								)
						);
		        $cAdmin->registerAdminMenuEntry($admin_menu_entry_path, 'index.php?s=cAdminUserUnit');
				}
		}

		private function checkIfDeletable() {
			$userUnitId = (int)$this->data['data']['id'];
	
			// Check if there are appointments assigned
			$iAppointment = new cAppointment();
			$status = $iAppointment->hasUserUnitAppointments($userUnitId);
	
			if($status == true) {
				header('Location: index.php?s=cAdminUserUnit&error=1001');
				die;
			}
		}
