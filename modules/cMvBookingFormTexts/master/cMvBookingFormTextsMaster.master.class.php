...
class cMvBookingFormTextsMaster extends cModule {
		///////////////////////////////////////////////////////////////////////////////////////////////////
		...
		public static function loadById($id) {
				...
				$db->setQuery('SELECT * FROM ' . $db->table('mv_booking_form_texts') . ' WHERE id = :id LIMIT 1;');
				$db->bind(':id', (int)$id);
				$result = $db->execute();
		...
		public static function loadList() {
				...
				$db->setQuery(
						'SELECT * ' .
						'FROM ' . $db->table('mv_booking_form_texts') . ' ' .
						'ORDER BY title;'
				);
				$result = $db->execute();
		...
		public static function createInDB($data) {
				...
				$db->useInstance('systemdb');
				$db->setQuery(
						'INSERT INTO ' . $db->table('mv_booking_form_texts') . ' ' .
		...
		public static function updateInDB($id, $data) {
				...
				$db->useInstance('systemdb');
				$db->setQuery(
						'UPDATE ' . $db->table('mv_booking_form_texts') . ' SET ' .
		...
		public static function deleteById($id) {
				...
				$db->useInstance('systemdb');
				$db->setQuery(
						'DELETE FROM ' . $db->table('mv_booking_form_texts') . ' ' .
						'WHERE ' .
								'id = :id'
				);
				$db->bind(':id', (int)$id);
