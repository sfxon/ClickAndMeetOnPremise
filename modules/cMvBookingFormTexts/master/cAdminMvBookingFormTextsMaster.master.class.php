...
class cAdminMvBookingFormTextsMaster extends cModule {
		var $template = 'admin_templates';
		var $navbar_title = 'Kalender-Texte';
		var $navbar_id = 0;
		var $errors = array();
		...
		public static function setExecutionalHooks() {
				...
				//check the rights..
				if(false === cAccount::adminrightCheck('cAdminMvBookingFormTexts', 'USE_MODULE', (int)$_SESSION['user_id'])) {
		...
		function process() {
				...
				$cAdmin->appendBreadcrumb('Admin Kalender-Texte', 'index.php?s=cAdminMvBookingFormTexts');
				...
				switch($this->action) {
						...
						case 'confirm_delete':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminMvBookingFormTexts&amp;action=delete&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'edit':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminMvBookingFormTexts&amp;action=update&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'new':
								$this->initData();
								$this->data['url'] = 'index.php?s=cAdminMvBookingFormTexts&amp;action=create';
								$cAdmin->appendBreadcrumb('Neu', '');
								$this->navbar_title = 'Neu';
								break;
						default:
		...
		private function delete() {
				...
				if($button_do_not_delete !== NULL && $button_do_not_delete === 'not_delete') {
						header('Location: index.php?s=cAdminMvBookingFormTexts&error=331');
						die;
				}
				
				//delete button
				if($button_delete !== NULL && $button_delete === 'delete') {
						cMvBookingFormTexts::deleteById($this->data['data']['id']);
						
						header('Location: index.php?s=cAdminMvBookingFormTexts&success=332');
						die;
				}
		...
		function initData() {
    		        $this->data['data']['id'] = 0;
     		        ...
     		        $this->data['data']['internal_identifier'] = '';
    ...
    ///////////////////////////////////////////////////////////////////
		...
		public function getTemplatePath() {
    		return 'modules/cMvBookingFormTexts/';
    }
		...
		public function update() {
				...
				if(0 == $id) {
						header('Location: index.php?s=admincAdminMvBookingFormTexts&error=305');
						die;
				}
        ...
        $data['id'] = $id;
                ...
                $data['title'] = core()->getPostVar('title');
     		        $data['internal_identifier'] = core()->getPostVar('internal_identifier');
     		        $data['field_value'] = core()->getPostVar('field_value');
     		
				
				if(NULL === cMvBookingFormTexts::loadById((int)$data['id'])) {
						header('Location: index.php?s=cAdminMvBookingFormTexts&error=306');
						die;
				}
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminMvBookingFormTexts&error=307');
						die;
				}
	
				header('Location: index.php?s=cAdminMvBookingFormTexts&action=edit&id=' . $id . '&success=308');
				die;
		}
		...
		function create() {
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminMvBookingFormTexts&error=309');
						die;
				}
	
				header('Location: index.php?s=cAdminMvBookingFormTexts&action=edit&id=' . $id . '&success=310');
				die;
		}
		...
		public function loadList() {
				return cMvBookingFormTexts::loadList();
		}
		...
		public function save($data) {
				...
				if(0 === $id) {
						return cMvBookingFormTexts::createInDB($data);
				}
				
				cMvBookingFormTexts::updateInDB($id, $data);
		...
		public function getContent() {
				$id = (int)core()->getGetVar('id');
				$this->data['data'] = cMvBookingFormTexts::loadById($id);
		}
    ...
    public static function setAdditionalHooks() {
		...
		public static function addMenuBarEntries() {
		        ...
		        $cAdmin->registerAdminMenuEntry($admin_menu_entry_path, 'index.php?s=cAdminMvBookingFormTexts');
				}
		}
