...
class cAdminTestMaster extends cModule {
		var $template = 'admin_templates';
		var $navbar_title = 'Test';
		var $navbar_id = 0;
		var $errors = array();
		...
		public static function setExecutionalHooks() {
				...
				//check the rights..
				if(false === cAccount::adminrightCheck('cAdminTest', 'USE_MODULE', (int)$_SESSION['user_id'])) {
		...
		function process() {
				...
				$cAdmin->appendBreadcrumb('Admin Test', 'index.php?s=cAdminTest');
				...
				switch($this->action) {
						...
						case 'confirm_delete':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminTest&amp;action=delete&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'edit':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminTest&amp;action=update&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'new':
								$this->initData();
								$this->data['url'] = 'index.php?s=cAdminTest&amp;action=create';
								$cAdmin->appendBreadcrumb('Neu', '');
								$this->navbar_title = 'Neu';
								break;
						default:
		...
		private function delete() {
				...
				if($button_do_not_delete !== NULL && $button_do_not_delete === 'not_delete') {
						header('Location: index.php?s=cAdminTest&error=331');
						die;
				}
				
				//delete button
				if($button_delete !== NULL && $button_delete === 'delete') {
						cTest::deleteById($this->data['data']['id']);
						
						header('Location: index.php?s=cAdminTest&success=332');
						die;
				}
    ...
    ///////////////////////////////////////////////////////////////////
		...
		public function update() {
				...
				if(0 == $id) {
						header('Location: index.php?s=admincAdminTest&error=305');
						die;
				}
        ...
        $data['id'] = $id;
                ...
                $data['title'] = core()->getPostVar('title');
     		        $data['testfeld1'] = (int)core()->getPostVar('testfeld1');
     		
				
				if(NULL === cTest::loadById((int)$data['id'])) {
						header('Location: index.php?s=cAdminTest&error=306');
						die;
				}
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminTest&error=307');
						die;
				}
	
				header('Location: index.php?s=cAdminTest&action=edit&id=' . $id . '&success=308');
				die;
		}
		...
		function create() {
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminTest&error=309');
						die;
				}
	
				header('Location: index.php?s=cAdminTest&action=edit&id=' . $id . '&success=310');
				die;
		}
		...
		public function save($data) {
				...
				if(0 === $id) {
						return cTest::createInDB($data);
				}
				
				cTest::updateInDB($id, $data);
		...
		public function getContent() {
				$id = (int)core()->getGetVar('id');
				$this->data['data'] = cTest::loadById($id);
		}
    ...
    public static function setAdditionalHooks() {
		...
		public static function addMenuBarEntries() {
		        ...
		        $cAdmin->registerAdminMenuEntry($admin_menu_entry_path, 'index.php?s=cAdminTest');
				}
		}
