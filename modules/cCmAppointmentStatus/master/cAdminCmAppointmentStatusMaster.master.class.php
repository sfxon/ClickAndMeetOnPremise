...
class cAdminCmAppointmentStatusMaster extends cModule {
		var $template = 'admin_templates';
		var $navbar_title = 'CmAppointmentStatus';
		var $navbar_id = 0;
		var $errors = array();
		...
		public static function setExecutionalHooks() {
				...
				//check the rights..
				if(false === cAccount::adminrightCheck('cAdminCmAppointmentStatus', 'USE_MODULE', (int)$_SESSION['user_id'])) {
		...
		function process() {
				...
				$cAdmin->appendBreadcrumb('Admin CmAppointmentStatus', 'index.php?s=cAdminCmAppointmentStatus');
				...
				switch($this->action) {
						...
						case 'confirm_delete':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminCmAppointmentStatus&amp;action=delete&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'edit':
								$this->initData();
								$this->getContent();
								$this->data['url'] = 'index.php?s=cAdminCmAppointmentStatus&amp;action=update&amp;id=' . (int)$this->data['data']['id'];
						...
						case 'new':
								$this->initData();
								$this->data['url'] = 'index.php?s=cAdminCmAppointmentStatus&amp;action=create';
								$cAdmin->appendBreadcrumb('Neu', '');
								$this->navbar_title = 'Neu';
								break;
						default:
		...
		private function delete() {
				...
				if($button_do_not_delete !== NULL && $button_do_not_delete === 'not_delete') {
						header('Location: index.php?s=cAdminCmAppointmentStatus&error=331');
						die;
				}
				
				//delete button
				if($button_delete !== NULL && $button_delete === 'delete') {
						cCmAppointmentStatus::deleteById($this->data['data']['id']);
						
						header('Location: index.php?s=cAdminCmAppointmentStatus&success=332');
						die;
				}
		...
		function initData() {
    		        $this->data['data']['id'] = 0;
     		        ...
     		        $this->data['data']['internal_required'] = 0;
     				}
    ...
    ///////////////////////////////////////////////////////////////////
		...
		public function getTemplatePath() {
    		return 'modules/cCmAppointmentStatus/';
    }
		...
		public function update() {
				...
				if(0 == $id) {
						header('Location: index.php?s=admincAdminCmAppointmentStatus&error=305');
						die;
				}
        ...
        $data['id'] = $id;
                ...
                $data['title'] = core()->getPostVar('title');
     		        ...
     		        $data['internal_required'] = (int)core()->getPostVar('internal_required');
     		
				
				if(NULL === cCmAppointmentStatus::loadById((int)$data['id'])) {
						header('Location: index.php?s=cAdminCmAppointmentStatus&error=306');
						die;
				}
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminCmAppointmentStatus&error=307');
						die;
				}
	
				header('Location: index.php?s=cAdminCmAppointmentStatus&action=edit&id=' . $id . '&success=308');
				die;
		}
		...
		function create() {
				...
				if(empty($id)) {
						header('Location: index.php?s=cAdminCmAppointmentStatus&error=309');
						die;
				}
	
				header('Location: index.php?s=cAdminCmAppointmentStatus&action=edit&id=' . $id . '&success=310');
				die;
		}
		...
		public function loadList() {
				return cCmAppointmentStatus::loadList();
		}
		...
		public function save($data) {
				...
				if(0 === $id) {
						return cCmAppointmentStatus::createInDB($data);
				}
				
				cCmAppointmentStatus::updateInDB($id, $data);
		...
		public function getContent() {
				$id = (int)core()->getGetVar('id');
				$this->data['data'] = cCmAppointmentStatus::loadById($id);
		}
    ...
    public static function setAdditionalHooks() {
		...
		public static function addMenuBarEntries() {
		        ...
		        $cAdmin->registerAdminMenuEntry($admin_menu_entry_path, 'index.php?s=cAdminCmAppointmentStatus');
				}
		}
